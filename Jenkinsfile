...
// which are part of the Masternodo repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="masternodo-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /masternodo-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "masternodopay-masternodo/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from masternodopay-masternodo/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'masternodopay-masternodo/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from masternodopay-masternodo/develop"
            }
          }

          if (hasCache) {
            sh "cd /masternodo-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /masternodo-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /masternodo-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /masternodo-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /masternodo-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /masternodo-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /masternodo-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
